"""Embedding store for managing document embedding state.

This module provides the EmbeddingStore domain manager, which handles
the lifecycle and state of document embedding providers and indices.
It communicates state changes via the event bus rather than direct
UI coupling.
"""
...
from ..events import EmbeddingStateChanged
...
class EmbeddingRuntimeState:
    ...
    Attributes:
        backend: The embedding backend type (e.g., "langchain", "sentence-transformers").
        model: The model name or identifier being used.
        provider_label: A human-readable label for the provider.
        status: Current status ("ready", "error", "disabled", "unavailable").
        detail: Additional detail about the current state.
        error: Error message if status is "error".
        mode: The embedding mode ("local", "same-api", "custom-api", "disabled").
    """
    ...
    @property
    def label(self) -> str:
        """Get a short display label for the embedding state."""
    ...
    def as_snapshot(self) -> dict[str, Any]:
        """Convert to a snapshot dictionary for telemetry/debugging."""
        return {
...
class EmbeddingValidationResult:
    """Result of validating an embedding provider."""
...
class EmbeddingValidator:
    ...
    def __init__(
        self,
        *,
        sample_text: str = "TinkerBell embeddings validation ping",
        timeout: float = 15.0,
    ) -> None:
    ...
    async def validate(
        ...
        *,
        ...
        """Validate that an embedding provider is functional.

        Args:
            provider: The embedding provider to validate.
            mode: The embedding mode ("local" uses embed_documents, others use embed_query).
...
class _LangChainProviderTemplate:
    ...
    def matches(self, model_name: str, *, forced_family: str | None = None) -> bool:
        """Check if this template matches the given model name."""
        if forced_family:
    ...
    def dimension_for(self, model_name: str) -> int | None:
        """Get the dimension for a specific model, if known."""
...
class ActivityCallback(Protocol):
    """Protocol for embedding activity callbacks."""

    def __call__(self, active: bool, detail: str | None) -> None:
        """Called when embedding activity state changes."""
        ...


class EmbeddingStore:
    """Domain manager for embedding runtime state.

    The EmbeddingStore manages the lifecycle of embedding providers and indices,
    emitting EmbeddingStateChanged events when the state transitions. It removes
    direct UI coupling by using the event bus for status updates.

    Attributes:
        state: The current embedding runtime state.
        index: The active document embedding index, or None if disabled.
    """

    __slots__ = (
        "_event_bus",
        "_cache_root",
        "_async_loop_resolver",
        "_background_task_runner",
        "_embedding_index",
        "_embedding_state",
        "_embedding_signature",
        "_embedding_snapshot_metadata",
        "_embedding_resource",
        "_validator",
        "_activity_callback",
    )
    ...
    def __init__(
        ...
        event_bus: EventBus,
        *,
        async_loop_resolver: Callable[[], asyncio.AbstractEventLoop | None] | None = None,
        background_task_runner: Callable[
        ...
        Args:
            cache_root: Root directory for embedding cache storage.
            event_bus: Event bus for publishing state change events.
            async_loop_resolver: Optional resolver for the async event loop.
            background_task_runner: Optional runner for background async tasks.
            activity_callback: Optional callback for embedding activity updates.
        """
        self._event_bus = event_bus
        self._cache_root = cache_root
        self._async_loop_resolver = async_loop_resolver or (lambda: None)
        self._background_task_runner = background_task_runner or (lambda _: None)
    ...
    # ------------------------------------------------------------------
    @property
    def state(self) -> EmbeddingRuntimeState:
        """Get the current embedding runtime state."""
        return self._embedding_state

    @property
    def index(self) -> DocumentEmbeddingIndex | None:
        """Get the active document embedding index, or None if disabled."""
    ...
    def configure(self, settings: Settings | None) -> None:
        """Configure the embedding runtime based on settings.

        This method handles the full lifecycle of embedding configuration:
        - Validates settings and determines the embedding mode
        - Tears down existing providers if settings changed
        - Builds new providers and indices as needed
        - Schedules validation for the new provider
        - Emits EmbeddingStateChanged events

        Args:
            settings: Application settings, or None to disable embeddings.
        """
        if settings is None:
        ...
        backend = self._normalize_backend(
            getattr(settings, "embedding_backend", "auto"),
            mode,
        )
        if backend == "disabled":
        ...
        model_name = (
            (getattr(settings, "embedding_model_name", "") or "").strip()
            or "text-embedding-3-large"
        )
        embedding_settings = self._build_embedding_settings(settings, metadata, mode)
        signature = self._embedding_settings_signature(
            embedding_settings,
            backend,
            model_name,
            metadata,
        ...
        self._teardown_embedding_runtime(
            reason="Reinitializing embedding backend",
            keep_status=True,
        )
        ...
        if index is None:
            error_state = EmbeddingRuntimeState(
                ...
                provider_label=provider_state.provider_label,
                status="error",
                error=f"Unable to initialize embedding cache under {self._cache_root}",
                mode=mode,
            )
    ...
    def shutdown(self) -> None:
        """Shutdown the embedding runtime and release resources.

        This method tears down the embedding runtime, closes any open
        indices, and disposes of resources. Call this during application
        shutdown.
        """
        self._teardown_embedding_runtime(
            reason="Shutdown",
    ...
    def apply_snapshot_metadata(self, snapshot: dict[str, Any]) -> None:
        """Apply embedding metadata to a snapshot dictionary.
    ...
    # ------------------------------------------------------------------
    # Internal: Mode and backend resolution
    # ------------------------------------------------------------------
    def _resolve_embedding_mode(self, metadata: Mapping[str, Any]) -> str:
        raw_value = str(
            metadata.get("embedding_mode") or DEFAULT_EMBEDDING_MODE
        ).strip().lower()
    ...
    def _embedding_settings_signature(
        ...
        provider_env_secrets = tuple(
            os.environ.get(var) for var in _LANGCHAIN_PROVIDER_ENV_VARS
        )
        return (
            backend,
            model,
    ...
    def _signature_for_embedding_api(metadata: Mapping[str, Any]) -> str | None:
        ...
        if secret:
            filtered["api_key_fingerprint"] = hashlib.sha256(
                secret.encode("utf-8")
            ).hexdigest()
        for key in (
            "base_url",
        ...
        headers = payload.get("default_headers")
        if isinstance(headers, Mapping) and headers:
            filtered["default_headers"] = {
                str(k): str(v)
                for k, v in sorted(headers.items(), key=lambda item: str(item[0]))
            }
        if not filtered:
            return None
    ...
    def _build_embedding_provider(
        ...
        if backend == "langchain":
            return self._build_langchain_embedding_provider(
                model_name,
                settings,
                metadata,
                mode,
    ...
    def _build_openai_embedding_provider(
        ...
        base_url = (
            (getattr(settings, "base_url", "") or "").strip() or OPENAI_API_BASE_URL
        )
        if not api_key:
    ...
    def _build_langchain_embedding_provider(
        ...
        try:
            embeddings, provider_template = self._instantiate_langchain_embeddings(
                model_name,
                settings,
                metadata,
            )
        ...
        model_detail = getattr(embeddings, "model", None) or getattr(
            embeddings,
            "model_name",
            None,
        )
    ...
    def _instantiate_langchain_embeddings(
        ...
        class_override = metadata_payload.get(
            "langchain_embeddings_class"
        ) or os.environ.get("TINKERBELL_LANGCHAIN_EMBEDDINGS_CLASS")
        raw_kwargs = metadata_payload.get(
            "langchain_embeddings_kwargs"
        ) or os.environ.get("TINKERBELL_LANGCHAIN_EMBEDDINGS_KWARGS")
        extra_kwargs = self._parse_langchain_kwargs(raw_kwargs)
        provider_template = self._detect_langchain_provider_template(
            model_name,
            metadata_payload,
        )
        params = dict(extra_kwargs)
        params.setdefault("model", model_name)
        self._apply_langchain_provider_template(
            params,
            provider_template,
            model_name,
            settings,
        ...
        if class_override:
            ...
            if not module_name or not attr_name:
                raise RuntimeError(
                    "LangChain embeddings class override must include module path, "
    ...
    def _apply_langchain_provider_template(
        ...
        metadata_base_override = str(
            metadata.get(f"{template.family}_base_url") or ""
        ).strip()
        configured_base_url = metadata_base_override or (
            getattr(settings, "base_url", "") or ""
        ).strip()
        if template.family != "openai" and template.default_base_url:
            if self._should_override_base_url(
                configured_base_url,
    ...
    def _should_override_base_url(current: str, candidate: str) -> bool:
        ...
        return (
            normalized_current == openai_default
            and normalized_candidate != openai_default
        )
    ...
    # ------------------------------------------------------------------
    # Internal: Index creation and validation
    # ------------------------------------------------------------------
    def _create_embedding_index(
        ...
        *,
        ...
        try:
            loop = self._async_loop_resolver()
            if loop is not None:
    ...
    def _schedule_validation(
        ...
        *,
        ...
        self._background_task_runner(_run)
    ...
    # ------------------------------------------------------------------
    # Internal: Teardown and state management
    # ------------------------------------------------------------------
    ...
    def _close_embedding_index(self, index: DocumentEmbeddingIndex) -> None:
        ...
        self._background_task_runner(async_close)
    ...
    def _handle_embedding_activity(self, active: bool, detail: str | None) -> None:
        if self._activity_callback is not None:
            try:
                self._activity_callback(active, detail)
            except Exception:
                LOGGER.debug(
                    "Failed to update embedding activity indicator",
                    exc_info=True,
                )
    ...
    def _set_embedding_state(
        ...
        *,
        ...
        snapshot = {
            k: v for k, v in state.as_snapshot().items() if v not in (None, "")
        }
        self._embedding_snapshot_metadata = snapshot

        # Emit event for state change (replaces direct status bar coupling)
        if not hide_status:
            self._event_bus.publish(
                EmbeddingStateChanged(
                    backend=state.backend,
                    status=state.status,
                    detail=state.detail or state.error,
                )
            )
