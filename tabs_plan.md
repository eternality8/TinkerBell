...
- **AI parity**: the chat assistant and LangGraph tooling must be able to reason about every open tab, including reading from one tab and writing into another when asked (e.g., summarize Tab A into Tab B) while defaulting to the active tab when no target is specified.
...
- AI tools (`document_snapshot`, `document_edit`, `search_replace`, `validate_snippet`) implicitly target this single editor via the shared bridge.
- Autosave + restore logic only tracks one unnamed draft (`unsaved_snapshot`) plus one dirty file (`unsaved_snapshots[path]`).
- Status bar, menus, and chat suggestions all reflect the lone editor state.
...
4. **Status bar & chat**: always describe the active tab (cursor info, document name). Chat history remains global regardless of the selected tab, with metadata badges identifying the source and target tabs for each AI turn so users can orchestrate cross-tab workflows without losing conversational continuity.
...
- Update `WindowAction` callbacks to reference the workspace:
  ...
  - Update undo/redo, toggle preview, etc., to scope to the active editor (most already do, but audit for any lingering references to `self._editor`).
...
- Expand AI tool tests (`tests/test_ai_tools.py`) to assert that `tab_id` is included in snapshot/edit payloads and that invalid `tab_id`s raise errors.
